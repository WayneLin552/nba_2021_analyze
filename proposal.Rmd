...
## Research topic
NBA is one of the most famous sports leagues in the world. In this final project, we will focus on data analysis of NBA regular season and playoffs in season 2021 to 2022. 

In regular season, we will analyze each team's performance base on conference. Performance will include: **(!!!LINHAO YU WRITE HERE!!!)**. Those features can be divided by offensive and defensive statistics. We will use some visualization techniques to check different conference's styles. Then determine if those styles have significant influence on teams' victory rate.
...
|==>**Format of data**: because there is no cvs file provided, we use *revest* package to read the data directly from html. So it will be several data frames and all features are numeric data types.
